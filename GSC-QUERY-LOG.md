...
| 2026-05-20 |  |  |  |  |  | 10:08 Asia/Shanghai public crawlability recheck: live `sitemap.xml` returns 200 with `Content-Type: application/xml`, Googlebot user-agent also receives 200, and `robots.txt` returns 200. Browser attempt to open GSC UI timed out, so the latest confirmed UI state remains the 2026-05-19 `Couldn't fetch` / `Sitemap could not be read` record. Current evidence still points to GSC processing/UI delay rather than a bad sitemap file. |
| 2026-05-20 | 375 | 14.6K | 2.6% | 8.1 | 11 | 14:xx Asia/Shanghai GSC UI check after login. Performance last update 4 hours ago, date range May 12-17, 2026: clicks rose from 364 to 375, impressions from 14.1K to 14.6K, CTR still 2.6%, avg position 8.1. Page indexing last update remains 2026-05-15: 11 indexed, 1 not indexed, only reason `Alternate page with proper canonical tag`, expected for `/index.html`. Sitemaps page reveals the submitted sitemap row is `/sitemap.xml/` with a trailing slash, Type Unknown, Submitted May 19, Status `Couldn't fetch`, Discovered pages 0. Public sitemap URL without trailing slash is valid, so next action is to submit exactly `sitemap.xml` (no leading slash, no trailing slash) in GSC. Browser automation could read the issue but GSC input actions timed out. |
| 2026-05-21 | 375 | 14.6K | 2.6% | 8.1 | 11 | Reopened GSC Sitemaps. The stale submitted row still shows `/sitemap.xml/`, Type Unknown, Submitted May 19, 2026, Status `Couldn't fetch`, Discovered pages 0. Confirmed the account is logged in and the Sitemaps page is accessible, but Google Search Console input automation repeatedly timed out when trying to type `sitemap.xml`. User should manually submit exact `sitemap.xml` in the Add a new sitemap field. |
| 2026-05-21 | 375 | 14.6K | 2.6% | 8.1 | 11 | User manually submitted exact `/sitemap.xml` in GSC Sitemaps. New row now shows `/sitemap.xml`, Type Unknown, Submitted May 21, 2026, Status `Couldn't fetch`, Discovered pages 0 immediately after submission. Public live check confirms `https://jodelilexize44-lgtm.github.io/subnautica-2-tools/sitemap.xml` returns 200 with `Content-Type: application/xml` and contains 28 URLs. Treat the immediate GSC warning as pending processing unless it remains unchanged after recheck. |
...
| 2026-05-20 | subnautica 2 full player hub / download / install / game content / progression / screenshots / forum | `index.html` |  |  |  | Homepage positioning | Strengthened homepage promise from tool directory to player hub, adding an "Everything Players Look For" table that routes download, setup, progression, map/content, media, and community needs without claiming a complete verified wiki. | Done |
| 2026-05-20 | subnautica 2 database / complete guide / all content | `guides/player-database.html`, `community.html` |  |  |  | Database coverage | Added a current coverage vs complete game database matrix so players can see ready lanes, missing verified data, and priority; updated community evidence CTA now that the contact page is live. | Done |
| 2026-05-20 | subnautica 2 database roadmap / screenshots / community submissions / game atlas | `guides/database-roadmap.html`, `guides/gallery.html`, `guides/game-content.html`, `community.html`, `index.html` |  |  |  | Player hub depth | Added a public database roadmap, homepage roadmap links, future featured gallery slots, screenshot browsing modes, world atlas planning, hidden player questions, weekly verification themes, and forum rollout phases. | Done |
| 2026-05-20 | reddit outreach landing gaps: game pass save, cross-platform disabled, steam deck black screen, random crash, gtx 1650 | `guides/save-backup.html`, `guides/co-op.html`, `guides/steam-deck.html`, `guides/crash-fix.html`, `tools/system-requirements.html` |  |  |  | Post-outreach landing quality | Added evidence-labeled sections that answer the exact player questions seen during Reddit outreach, while keeping unverified fixes marked as community report or needs current-build test. | Done |
| 2026-05-20 | player report submission / community evidence / bug report / screenshot submission | `community-report.html`, `community.html`, `index.html` |  |  |  | Community conversion | Added a structured player report page and linked it from homepage, community, crash, save, co-op, Steam Deck, and specs pages so outreach traffic can become source-labeled evidence. | Done |
